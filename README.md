...
Use **Save capsule** to persist the draft in `chrome.storage.local`, choose a **Bridge preset**, use **Copy context** to copy that deterministic preset output, use **Copy Markdown** for the existing full Markdown rendering, use **Copy source** for a concise citation with title, optional project, source conversation, selected turn IDs, and updated time, or **Download capsule** for a local `.md` file. Supported bridge presets are **Plain Markdown**, **Generic AI context**, **ChatGPT Project-style context**, **Claude Project-style context**, and **Gemini context**. The Gemini preset is optimized for manual paste into Gemini or Gemini workspace-style sessions, not UI automation. These presets and source citations are local text renderings built from the saved Work Capsule fields; they do not call provider APIs, upload files, automate Projects or Gemini workspaces, update provider-side memory, or add any Chrome permissions. Reopened capsules can update or clear the project label, and the local `workCapsules:v1` index keeps that label for reuse. After capture, the popup also loads the index and shows a compact **Library** that groups the five most recent saved capsules by project label when available, otherwise by source conversation title for local retrieval. Library rows show title, goal/source, updated time, and provide **Reopen**, **Copy context**, **Copy source**, and **Remove** actions; these actions load or remove the stored capsule body by ID, so a selected bridge preset or source citation can be reused even when the current conversation is different and saved capsules from other conversations can still be removed locally. When a saved capsule already matches the captured conversation URL, the existing recent capsule panel still appears with **Reopen**, **Copy source**, and **Delete**; **Delete** removes that saved capsule from local storage after confirmation. If no messages are checked, capsule creation shows the same selection error as export: `Select at least one message to export`. The loop is deterministic and local-only; it does not call a backend, cloud service, analytics endpoint, remote LLM, `fetch`, or `XMLHttpRequest`.
